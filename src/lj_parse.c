...
static void parse_chunk_one_stmt(LexState *ls);
...
/* Parse continue. */
static void parse_continue(LexState *ls, BCLine line)
{
	//ls->fs->bl->flags |= FSCOPE_BREAK;
...
static void parse_for_num(LexState *ls, GCstr *varname, BCLine line, int *one_stmt)
{
	...
	*one_stmt = 0;
	...
	// Continue label
	MSize idx = gola_new(ls, NAME_CONTINUE, VSTACK_LABEL, loopend);
	ls->vtop = idx;
	gola_resolve(ls, &bl, idx);
}
...
static void parse_for_iter(LexState *ls, GCstr *indexname, int *one_stmt)
{
	...
	// Continue label
	MSize idx = gola_new(ls, NAME_CONTINUE, VSTACK_LABEL, loop);
	ls->vtop = idx;
	gola_resolve(ls, &bl, idx);
}
...
// Read one stmt for if's without then
static void parse_chunk_one_stmt(LexState *ls)
{
	synlevel_begin(ls);
	parse_stmt(ls);
